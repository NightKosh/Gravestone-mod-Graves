...
public class GraveInventory implements IInventory {
    ...
    protected List<ItemStack> items = new ArrayList<ItemStack>(DEFAULT_INVENTORY_SIZE);
    ...
    public void readItems(NBTTagCompound nbtTag) {
        ...
        items = new ArrayList<ItemStack>(DEFAULT_INVENTORY_SIZE);
    ...
    public boolean isUseableByPlayer(EntityPlayer player) {
        return player.worldObj.getTileEntity(this.tileEntity.xCoord, this.tileEntity.yCoord, this.tileEntity.zCoord) == this.tileEntity &&
                player.getDistanceSq(this.tileEntity.xCoord + 0.5, this.tileEntity.yCoord + 0.5, this.tileEntity.zCoord + 0.5) < 64;
    }
            ...
            switch (GraveStoneConfig.graveItemsCount) {
                case 0:
                    for (ItemStack item : items) {
                        dropItem(item, tileEntity.getWorldObj(), tileEntity.xCoord, tileEntity.yCoord, tileEntity.zCoord);
                    }
                    break;
                case 40:
                    for (ItemStack item : items) {
    ...
    public void dropItem(int slot) {
        dropItem(items.get(slot), tileEntity.getWorldObj(), tileEntity.xCoord, tileEntity.yCoord, tileEntity.zCoord);
    }
    ...
    public void setRandomGraveContent(IInventory inventory, Random random, boolean isPetGrave, boolean allLoot) {
        ...
        if (isPetGrave) {
            ...
            if (tileEntity.getGraveType().ordinal() != 3 && tileEntity.getGraveType().ordinal() != 4) {
